...
!       call calc_permutation(dimids(1:2), arraydimids, permutation, ispermuted)
  ...
  subroutine infld_real_4d_4d(varname, ncid, dimname1, dimname2, dimname3, dimname4, &
    ...
    character(len=*),  intent(in)     :: dimname2 ! name of 2nd array dimensions of field on file (array order)
    character(len=*),  intent(in)     :: dimname3 ! name of 3rd array dimensions of field on file (array order)
    character(len=*),  intent(in)     :: dimname4 ! name of 4th array dimensions of field on file (array order)
    integer,           intent(in)     :: dim1b    ! start of first  dimension of array to be returned
    integer,           intent(in)     :: dim1e    ! end   of first  dimension of array to be returned
    integer,           intent(in)     :: dim2b    ! start of second dimension of array to be returned
    integer,           intent(in)     :: dim2e    ! end   of second dimension of array to be returned
    integer,           intent(in)     :: dim3b    ! start of third  dimension of array to be returned
    integer,           intent(in)     :: dim3e    ! end   of third  dimension of array to be returned
    integer,           intent(in)     :: dim4b    ! start of 4th  dimension of array to be returned
    integer,           intent(in)     :: dim4e    ! end   of 4th  dimension of array to be returned
    ...
    ! Offsets for reading global variables
    integer                   :: strt(4)     ! start lon, lev, lat indices for netcdf 4-d
    integer                   :: cnt (4)     ! lon, lat counts for netcdf 4-d
    character(len=PIO_MAX_NAME) :: tmpname

    real(r8), pointer         :: tmp4d(:,:,:,:) ! input data for permutation

    logical                   :: readvar_tmp ! if true, variable is on tape
    character(len=*), parameter :: subname='INFLD_REAL_4D_4D' ! subroutine name
    character(len=128)        :: errormsg
    ...
    ! Should we be using a different interface?
    if ((trim(dimname1) == trim(dimname2)) .or. (len_trim(dimname2) == 0)) then
      write(errormsg, *)': infld custom error message, you messed up Chapman; 00 "'
    else if ((trim(dimname1) == trim(dimname3)) .or. (len_trim(dimname3) == 0)) then
      write(errormsg, *)': infld custom error message, you messed up Chapman; 01 "'
    else
      ...
      if (readvar_tmp) then
        if (debug .and. masterproc) then
          write(iulog, '(2a,9(i0,a))') trim(subname),': field(',dim1b,':',dim1e, &
          ',',dim2b,':',dim2e,',',dim3b,':',dim3e,',',dim4b,':',dim4e,'),       &
        ...
        !write(iulog,*) 'dimension names,',dimname1,dimname2,dimname3,dimname4
        !write(iulog,*) size(field, 1)
        !write(iulog,*) size(field, 2)
        !write(iulog,*) size(field, 3)
        !write(iulog,*) size(field, 4)
        ...
        if (single_column) then
          ...
          if (ispermuted) then
            call cam_permute_array(strt, permutation)
            call cam_permute_array(cnt, permutation)
            allocate(tmp4d(1:cnt(1), 1:cnt(2), 1:cnt(3),1:cnt(4)))
            ierr = pio_get_var(ncid, varid, strt, cnt, tmp4d)
            do l = dim4b, dim4e
            do k = dim3b, dim3e
              do j = dim2b, dim2e
                do i = dim1b, dim1e
                  ...
                  strt(4) = l - dim4b + 1
                  call cam_permute_array(strt, permutation)
                  field(i,j,k,l) = tmp4d(strt(1), strt(2), strt(3),strt(4))
                end do
              end do
            end do
            end do
          ...
          !write(iulog,*) 'in conditional ncdio_atm'
          ! All distributed array processing
          call cam_grid_get_decomp(grid_id, arraydimsize, dimlens(1:pdims),  &
          ...
          call pio_read_darray(ncid, varid, iodesc, field, ierr)
          !write(iulog,*) 'varid::',varid
          !write(iulog,*) 'wabbafield::',varname,field(10,10,4:5,:)
        end if ! end of single column
    ...
    end if ! end of call infld_real_4d_4d instead
